            ...
            <div class="card">
                ...
                <table class="table small">
                  ...
                  @foreach($records as $record)
                    <tr @if ($record->sold) style="background-color: #d3d3d3; text-decoration: line-through;" 
                    @elseif ($record->lost) style="background-color: #800080; text-decoration: line-through; "@endif >
                      {{-- <td>{{ $record->id }}</td> --}}
                      <td>{{ $record->kind }}</td>
                      ...
                      <td><a href="{{route('record.edit',['id'=>$record->id])}}" class = "btn btn-sm"><i class="bi bi-pencil-square"></i></a></td>
                      <td><a href="{{route('record.destroy',['id'=>$record->id])}}" class = "btn btn-sm" onclick="return confirm('Delete {{ $record->artist->name }} - {{ $record->title }}?')"><i class="bi bi-trash"></i></a></td>
                    </tr>
                  @endforeach
                </table>
                @else
